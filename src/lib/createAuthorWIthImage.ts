...
export type CreateAuthorWithImageInput = z.infer<typeof addAuthorSchema>;

// Server Action to Create Author with Optional Image Upload
export async function createAuthorWithImage(
  data: CreateAuthorWithImageInput, 
  file: File | null
) {
  try {
    // Validate the input data
    const validatedData = addAuthorSchema.parse(data);
    ...
    // Create the author with the processed data
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Une erreur s'est produite lors de la création de l'auteur",
    };
  }
}
