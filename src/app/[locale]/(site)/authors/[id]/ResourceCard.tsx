...
export default function ResourceCard({ resource }: ResourceCardProps) {
  ...
  const [resourcePath, setResourcePath] = useState(resource.path || null);
  const videoRef = useRef<HTMLVideoElement | null>(null);
  const audioRef = useRef<HTMLAudioElement | null>(null);
  const isBrowser = typeof window !== 'undefined';

  const getTypeColors = (typeLabel: string | undefined) => {
    if (!typeLabel) return CATEGORY_COLORS.default;
  ...
  const getTypeIcon = () => {
    const normalizedType = resource.type?.label 
      ? normalizeTypeLabel(resource.type.label)
      : "default";
    const iconMap = {
      image: ImageIcon,
      video: Film,
  ...
  useEffect(() => {
    ...
    if (resource.path) {
      setResourcePath(getApiPath(resource.path));
    }

    // Safety check to ensure resource.type exists
    if (!resource.type || !resource.type.label) {
      return;
    }
    ...
    if (isVideo && resourcePath && !videoThumbnail) {
    ...
    if (isAudio && resourcePath && !audioDuration) {
  ...
  }, [resource?.type?.label, resourcePath, videoThumbnail, audioDuration, isBrowser, resource.path]);
          ...
          <CardFooter className="border-t border-website-primary/30 mt-3 pt-3 pb-1 text-sm text-website-text/80 flex justify-between">
            ...
            <div className="flex items-center gap-3">
              <span className="flex items-center gap-1">
                ...
                {Number(resource.views || 0).toLocaleString()}
              </span>
              <span className="flex items-center gap-1">
                <ThumbsUp className="h-3.5 w-3.5" />
                {Number(resource.upvotes || 0).toLocaleString()}
              </span>
            </div>
          </CardFooter>
        </div>
